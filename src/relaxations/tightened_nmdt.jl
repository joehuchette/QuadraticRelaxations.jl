struct TightenedNMDT <: AbstractRelaxation
    num_layers::Int
end

function reformulate_unary_quadratic_term!(
    model::MOI.ModelLike,
    reformulation::Reformulation{TightenedNMDT},
    _x::VI,
    _y::VI,
    _x_l::Float64,
    _x_u::Float64,
    ...
    MOI.add_constraint(model, Δz - 2.0^(-L) * Δx, MOI.LessThan(0.0))
    MOI.add_constraint(model, Δz, MOI.GreaterThan(0.0))
    MOIU.normalize_and_add_constraint(
        model,
        Δz - 2.0^(-L + 1) * (Δx - 2.0^(-L)) - 2.0^(-2L),
        MOI.GreaterThan(0.0),
    )
    # Only adding one cut corresponding to x̃_1 = 2^(-L-1)
    xt_1 = 2.0^(-L - 1)
    MOIU.normalize_and_add_constraint(
        model,
        Δz - 2.0 * xt_1 * (Δx - xt_1) - xt_1^2,
        MOI.GreaterThan(0.0),
    )
    return α_v
end
